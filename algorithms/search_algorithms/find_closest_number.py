...
def find_closest_num(A, target):
    ...
    while low <= high:
        ...
        # of the list.
        if mid+1 < len(A):
            max_diff_right = abs(A[mid + 1] - target)
        max_diff_left = abs(A[mid - 1] - target)

        # Check if the absolute value between left
        # and right elements are smaller than any
        # seen prior.
        if max_diff_left < max_diff:
            max_diff = max_diff_left
            closest_num = A[mid - 1]

        if max_diff_right < max_diff:
            max_diff = max_diff_right
            closest_num = A[mid + 1]
